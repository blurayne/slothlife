...
- [x] **Icy branches in winter** — per-branch `.icy` flag rolled in Jan-Feb (or via the dev FORCE ICY toggle); gripped limb on an icy branch slowly drifts toward the tip, slips past the end → existing fall path; cool-blue gradient on the branch's upper edge. (Cold-rain trigger removed in a follow-up — rain alone shouldn't freeze branches.) — [plan](.claude/plans/icy-branches.md)
